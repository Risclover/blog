...
const TableOfContents = (props: Props) => {
  ...
  useEffect(() => {
    const ids = props.headings.map((e) => e.slug);
    ...
    props.headings.forEach((heading: any) => observer.observe(heading));
  }, []);
  return (
    <div className="toc min-w-fit ml-20 hidden lg:block">
      <h1 className="uppercase font-medium tracking-widest mb-4">
        Table of Contents
      </h1>
      <ul>
        {props.headings.map(
